            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if is_inception and phase == 'train':
                        ...
                        aux_outputs1, aux_outputs2, outputs = model(inputs)
                        loss1 = criterion(outputs, labels)
                        loss2 = criterion(aux_outputs1, labels)
                        loss3 = criterion(aux_outputs2, labels)
                        loss = loss3 + 0.3 * loss2 + 0.3 * loss1 
                    else:
...
# without LRN 
google_net1 = GoogLeNet(num_classes = 10)
google_net1 = google_net1.to(device)

params_to_update = google_net1.parameters()
...
# with LRN 
google_net2 = torch.hub.load('pytorch/vision:v0.6.0', 'googlenet', pretrained=True)
google_net2 = google_net2.to(device)
params_to_update = google_net2.parameters()
...
val_acc = np.max(val_acc_history)
df = df.append(dict(
    model='GoogLeNet-Pretained',
    val_acc_history=val_acc,
    time_elapsed=time_elapsed,
    n_params = total_params,
), ignore_index=True)

df.to_csv('GoogLeNetResult.csv', index=False)
