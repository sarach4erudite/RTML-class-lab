...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, weights_name='weight_save', is_inception=False):
    ...
    return model, val_acc_history, loss_acc_history, time_elapsed
...
from AlexNet_1 import AlexNetModule
# without LRN 
alexnet_module_1 = AlexNetModule(10)
alexnet_module_1 = alexnet_module_1.to(device)

params_to_update = alexnet_module_1.parameters()
criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(params_to_update, lr=0.001, momentum=0.9)

total_params = sum(p.numel() for p in alexnet_module_1.parameters())
alexnet_module_1, val_acc_history, loss_acc_history, time_elapsed = train_model(alexnet_module_1, dataloaders, criterion, optimizer, 10, 'alex_sequential_lr_0.001_bestsofar')
...
from AlexNet_2 import AlexNetModule as AlexNetModule_LRN
# with LRN 
alexnet_module_2 = AlexNetModule_LRN(10)
alexnet_module_2 = alexnet_module_2.to(device)
params_to_update = alexnet_module_2.parameters()

criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(params_to_update, lr=0.001, momentum=0.9)

total_params = sum(p.numel() for p in alexnet_module_2.parameters())
alexnet_module_2, val_acc_history, loss_acc_history, time_elapsed = train_model(alexnet_module_2, dataloaders, criterion, optimizer, 10, 'alex_sequential_lr_0.001_bestsofar')
...
df.to_csv('alexnet_result.csv', index=False)
